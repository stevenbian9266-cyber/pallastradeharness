...
| Version | Highlight |
|---|---|
| **v1.10.0** | **MCP mechanism integration ([RFC-0004](docs/rfc/0004-mcp-mechanism.md)) — Phase 0 + Phase 1**: command registry kernel (single source of truth for CLI/MCP) + gate data-function extraction; zero-install `harness-mcp` entry (root chain: `--root` > `HARNESS_ROOT` > roots negotiation > cwd; 20 L1 tools; `isError` envelope; human-WAIT checks cannot be cleared through MCP); `harness mcp:config` generates access configs for VS Code / Cursor / Claude Code / Claude Desktop / Codex (spec pinned by default); new `docs/mcp.md` guide; 309/309 tests passing |
| **v1.9.0** | ⚠️ [auto-generated, please polish] `gate --quiet`：只输出 check 计数 + 必读提示（默认全量保留）；`config.output.gateListVerbose=false` 等价降档 |

...
const RULES = [
  ...
  { id: 'SEC-003', severity: 'error', name: 'admin-api-secret-key',
    pattern: /sk_[1-9A-HJ-NP-Za-km-z]{24}/,
    message: 'Admin API secret key (sk_…). Never commit.', fix: 'Use env var; keys live in DB only.',
  ...
  { id: 'DNG-002', severity: 'error', name: 'drop-table',
    pattern: /DROP\s+TABLE\s+(?:IF\s+EXISTS\s+)?["`]?[A-Za-z0-9_]+/i,
    message: 'DROP TABLE in committed code. Physically blocked.', fix: 'Remove; use migrations.' },
  { id: 'DNG-003', severity: 'error', name: 'delete-from-core-table',
    pattern: /DELETE\s+FROM\s+["`]?[A-Za-z0-9_]*(?:orders|products|customers)\b/i,
    message: 'Mass DELETE against a core business table. Physically blocked.', fix: 'Remove; use scoped destruction.' },
